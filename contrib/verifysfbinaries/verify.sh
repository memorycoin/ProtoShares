...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'memoryoin-',
...
if [ $? -ne 0 ]; then
   ...
   echo "[memorycoin-]<version>-[rc[0-9]] (example: memorycoin-0.7.1-rc1)"
   echo "wget output:"
